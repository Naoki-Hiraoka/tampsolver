...
namespace multicontact_controller {
  bool MultiContactFootCoords::calcRootOdometry(cnoid::Body* robot, std::vector<std::shared_ptr<ContactPointMCFC> >& contactPoints){
    ...
    for(size_t loop=0;loop<loopNum_;loop++){
      ...
      for(size_t i=0;i<contactPoints.size();i++){
        const cnoid::Position& T_ref = contactPoints[i]->T_init();
  ...
  bool MultiContactFootCoords::setRootOdom(cnoid::Body* robot, const cnoid::Position& odomT){
  ...
  cnoid::Position MultiContactFootCoords::getOriginCoords(){
