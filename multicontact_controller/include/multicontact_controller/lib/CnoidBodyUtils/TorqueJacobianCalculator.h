...
namespace multicontact_controller{
  namespace cnoidbodyutils{
    ...
    public:
      ...
      Eigen::SparseMatrix<double,Eigen::RowMajor> calcDJw(std::vector<std::shared_ptr<ContactPoint> >& contactPoints);
      template <typename T>
      Eigen::SparseMatrix<double,Eigen::RowMajor> calcDJw(std::vector<std::shared_ptr<T> >& contactPoints){
    ...
    protected:
      ...
      // DJwはcontactpointに依存するのでキャッシュ不可
    };
